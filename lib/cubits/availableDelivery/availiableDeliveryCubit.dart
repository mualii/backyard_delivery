...
class AvailiableDeliveryCubit extends Cubit<AvailibaleDeliveryState> {
  ...
  getDeliveryList(BuildContext context) {
    ...
    try {
      availableList = [];
      DioHelper.getData(
          endpoint: "/api/driver/delivery/active",
          context: context,
          setParamars: {"page": "1"}).then((respone) {
      ...
      emit(AvailibaleDeliveryErrorState());
    }
  }
  ...
  loadMoreData(BuildContext context, int page) {
    ...
    try {
      DioHelper.getData(
          endpoint: "/api/driver/delivery/active",
          context: context,
          setParamars: {"page": "${page.toString()}"}).then((respone) {
        availaibleDelivery = AvailaibleDelivery.fromJson(respone);
  ...
  getDetails(BuildContext context, int id) {
    ...
    try {
      DioHelper.getData(
              endpoint: "/api/driver/delivery/details/${id}",
              setParamars: {},
              context: context)
          .then((reposne) async {
